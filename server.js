...
app.put('/api/distribuciones/:id', async (req, res) => {
  try {
    ...
    if (cantidadExtra !== undefined && cantidadExtra !== null && cantidadExtra !== '') {
      ...
      const disponibles = bolsas.reduce((sum, b) => {
        const usadas = (b.piezasVendidas || 0) + (b.piezasEntregadas || 0);
    ...
    const distribucion = await Distribucion.findByIdAndUpdate(
      req.params.id,
      {
        ...
        montoDevuelto: montoDevuelto !== undefined ? +montoDevuelto : previa.montoDevuelto,
        notas,
        pagado: (montoDevuelto !== undefined ? +montoDevuelto : previa.montoDevuelto) >= nuevasGanancias,
      },
      { new: true }
    );
    await distribucion.populate('candyId');
...
app.get('/api/semanas/:id', async (req, res) => {
  try {
    ...
    const ventas = await Venta.find({
      fecha: { $gte: semana.fechaInicio, $lte: semana.fechaFin },
    }).sort('fecha');
    res.json({ semana, ventas });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
